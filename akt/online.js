(function () {
  ...
  var Defined = {
    api: 'lampac',
    localhost: (window.LAMPAC_SRV || 'http://31.129.234.181/'),
    apn: ''
  };
  var TRACKER_URL = '';
...
var hostkey = 'http://31.129.234.181'.replace('http://', '').replace('https://', '');
...
window.rch_nws[hostkey].typeInvoke = function rchtypeInvoke(host, call) {
  if (!window.rch_nws[hostkey].startTypeInvoke) {
    ...
    if (Lampa.Platform.is('android') || Lampa.Platform.is('tizen')) check(true);
      ...
      net.silent('http://31.129.234.181'.indexOf(location.host) >= 0 ? 'https://github.com/' : host + '/cors/check', function() {
        check(true);
      }, function() {
        check(false);
      }, false, {
...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  window.rch_nws[hostkey].typeInvoke('http://31.129.234.181', function() {
    ...
    client.on("RchClient", function(rchId, url, data, headers, returnHeaders) {
	  ...
	  function sendResult(uri, html) {
	    $.ajax({
	      url: 'http://31.129.234.181/rch/' + uri + '?id=' + rchId,
	      type: 'POST',
	      data: html,
	      async: true,
	      cache: false,
  ...
  function rchRun(json, call) {
    if (typeof NativeWsClient == 'undefined') {
      Lampa.Utils.putScript(["http://31.129.234.181/js/nws-client-es5.js?v21042026"], function() {}, false, function() {
        rchInvoke(json, call);
      }, true);
    } else {
      rchInvoke(json, call);
  ...
  function trackLaunch(movie, balanser, file) {
    ...
    var token = account && account.token || Lampa.Storage.get('token', '');

    if (!TRACKER_URL || !Number.isInteger(userId) || !movie || !movie.id || !balanser) return;
  ...
  function component(object) {
    ...
    if (balansers_with_search == undefined) {
      ...
      network.silent(
        account('http://31.129.234.181/lite/withsearch'),
        function (json) {
