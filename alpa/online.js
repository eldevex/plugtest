...
(function(){
  ...
  var Defined = {
    api: 'lampac',
    localhost: (window.ALCOPAC_SRV||"http://31.129.234.181")+'/',
    apn: ''
  };
...
var hostkey = (window.ALCOPAC_SRV||"http://31.129.234.181").replace('http://', '').replace('https://', '');
...
window.rch_nws[hostkey].typeInvoke = function rchtypeInvoke(host, call) {
  if (!window.rch_nws[hostkey].startTypeInvoke) {
    ...
    if (Lampa.Platform.is('android') || Lampa.Platform.is('tizen')) check(true);
      ...
      net.silent((window.ALCOPAC_SRV||"http://31.129.234.181").indexOf(location.host) >= 0 ? 'https://github.com/' : host + '/cors/check', function() {
        check(true);
      }, function() {
        check(false);
      }, false, {
...
window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
  window.rch_nws[hostkey].typeInvoke((window.ALCOPAC_SRV||"http://31.129.234.181"), function() {
    ...
    client.on("RchClient", function(rchId, url, data, headers, returnHeaders) {
	  ...
	  function sendResult(uri, html) {
	    $.ajax({
	      url: (window.ALCOPAC_SRV||"http://31.129.234.181")+'/rch/' + uri + '?id=' + rchId,
	      type: 'POST',
	      data: html,
	      async: true,
	      cache: false,
  ...
  function rchRun(json, call) {
    if (typeof NativeWsClient == 'undefined') {
      Lampa.Utils.putScript([(window.ALCOPAC_SRV||"http://31.129.234.181")+"/js/nws-client-es5.js?v18112025"], function() {}, false, function() {
        rchInvoke(json, call);
      }, true);
    } else {
      rchInvoke(json, call);
  ...
  function component(object) {
    ...
    if (balansers_with_search == undefined) {
      ...
      network.silent(account((window.ALCOPAC_SRV||"http://31.129.234.181")+'/lite/withsearch'), function(json) {
  ...
  if (false) {
    if (Platform.is('browser') && location.host.indexOf("127.0.0.1") !== -1) {
      ...
      $.get((window.ALCOPAC_SRV||"http://31.129.234.181")+'/player-inner/' + element.url);
      return;
    }
